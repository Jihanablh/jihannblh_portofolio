...
export default function CertificateModal({ certificate, onClose }) {
  return (
    <motion.div className="fixed inset-0 z-[90] flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      >
        <button className="absolute right-4 top-4 z-20 grid h-11 w-11 place-items-center rounded-full border border-white/10 bg-black/45 text-white backdrop-blur-xl transition hover:bg-white/15" onClick={onClose} aria-label="Close">
          <X size={20} />
        </button>
        <div className="grid max-h-[92vh] overflow-y-auto lg:grid-cols-[1.15fr_.85fr]">
          ...
          <div className="p-6 sm:p-8">
            <span className="inline-flex rounded-full border border-cyanx/20 bg-cyanx/10 px-4 py-2 font-mono text-[11px] font-bold uppercase tracking-[0.18em] text-cyan-100">
              {certificate.category}
            </span>
            <h3 className="mt-5 font-display text-3xl font-bold leading-tight text-white">{certificate.title}</h3>
            <p className="mt-3 text-slate-400">{certificate.issuer} • {certificate.year}</p>
            <p className="mt-6 text-base leading-8 text-slate-300">{certificate.description}</p>
            <a href={certificate.link} target="_blank" rel="noreferrer" className="mt-8 inline-flex items-center gap-2 rounded-full bg-cyanx px-5 py-3 text-sm font-bold text-midnight transition hover:bg-white">
